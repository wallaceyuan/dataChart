	...
	function Slider(opts){
		...
		this.item = opts.item;
		this.idx = opts.index;
		this.len = this.item.parents('.pics').find('li').length;
		//构造三步奏
		this.init();
		this.renderDOM();
		this.bindDOM();
...
/*		if(window.innerWidth > 640){
	...
	Slider.prototype.renderDOM = function(){
		...
		//根据元素的
		for(var i = 0; i< len;i++){
			var img  = item.parents('.pics').find('img').eq(i).attr('src');
			...
			if(ImageObj.complete){
				ImageObj.onload = function(){
					...
					li.style.webkitTransform = 'translate3d('+ (num-index)*scaleW +'px, 0, 0)';
					if(item){
						if(imgheight/imgwidth > this.radio){
							li.innerHTML = '<img height="'+ window.innerHeight +'" src="'+ imgsrc +'">';
						}else{
							li.innerHTML = '<img width="'+ window.innerWidth +'" src="'+ imgsrc +'">';
						}
					}
		...
		$('#canvas #J-curPage').html(index+1);
		$('#canvas #J-pageCount').html("/"+len);
		$('#canvas').on('click',function(e){
			...
			setTimeout(function(){
				...
				setTimeout(function(){
					$('#canvas').css('display','none').css('z-index','-1');
				},300);
			},500);
			//$('.pics li').bind('tap');
		});
